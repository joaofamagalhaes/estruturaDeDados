...
Matriz *criaMatriz(int lin, int col) {
    ...
    mat->lin = lin;
    mat->col = col;
    mat->dados = malloc(lin * sizeof(int *));
    if (mat->dados == NULL) {
        free(mat);
        return NULL;
    }
    ...
    for (int i = 0; i < lin; i++) {
        mat->dados[i] = malloc(col * sizeof(int));
        if (mat->dados[i] == NULL) {
            for (int j = 0; j < i; j++) free(mat->dados[j]);
            free(mat->dados);
            free(mat);
            return NULL;
        }
...
int consultaElem(Matriz *mat, int *p, int lin, int col) {
    if (mat != NULL && p != NULL && lin >= 0 && lin < mat->lin && col >= 0 && col < mat->col) {
        *p = mat->dados[lin][col];
        return 1;
    }
    return 0;
